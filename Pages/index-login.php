<!DOCTYPE html>
<html lang="en">
    <head>
        <title>ระบบจองเวลากิจกรรม</title>
        <meta charset="utf-8">
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <link rel="stylesheet" href="../Bootstrap/css/bootstrap.min.css"/>
        <link rel="stylesheet" href="https://www.w3schools.com/w3css/4/w3.css">
        <link rel="stylesheet" href="../css/Script.css">
        <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.2.1/jquery.min.js"></script>
        <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.12.6/umd/popper.min.js"></script>
        <script src="https://maxcdn.bootstrapcdn.com/bootstrap/4.0.0-beta.2/js/bootstrap.min.js"></script>
    </head>
    <body>
        <div class="container-fluid">
            <div class="row">
                <div class="col-sm-7">
                    <h1>Welcome to Time Register System</h1>
                    <p>ระบบจองเวลาสอนสำหรับโรงเรียนจุฬาภรณราชวิทยาลัย ปทุมธานีนั้นสร้างขึ้นเพื่ออำนวยความสะดวกให้แก่อาจารย์ผู้สอนที่ต้องการสอนนักเรียนเสริมเพิ่มเติมจากเวลาเดิม โดยท่านสามารถจองเวลาสอนและสถานที่ได้ตามความพึงพอใจของท่าน</p>
                </div>
                <div class="col-sm-5">
                    <div class="btn btn-group-vertical">
                        <a class="btn btn-block btn-primary btn-lg" href="Booking.php" role="button">จองเวลาเรียน</a>
                        <a class="btn btn-block btn-secondary" href="Check.php" role="button">ตรวจสอบเวลาสอน</a>
                        <a class="btn btn-block btn-secondary" href="Register.php" role="button">สมัครสมาชิก</a>
                        <a class="btn btn-block btn-secondary" href="Login.php" role="button">ออกจากระบบ</a>
                        <a class="btn btn-block btn-secondary disabled" href="#" role="button">วิธีการใช้งาน</a>
                    </div>
                    <div class="row">
                        <div class="col-sm-12" id="footer">
                            <p style="font-size:.5em;">โรงเรียนจุฬาภรณราชวิทยาลัย ปทุมธานี 2017</p>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <footer>
             @2017 Create by <a href="https://web.facebook.com/profile.php?id=100001721779795">Posanice</a> <a href="https://web.facebook.com/Raliciel.Lolipopcron">Raliciel</a> and <a href="https://web.facebook.com/jasonnonstop10">Jasonnonstop10</a> 
        </footer>
    </body>
</html>
